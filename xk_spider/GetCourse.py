...
class GetCourse:
    ...
    def judge(self, course_name, teacher, classID,key='', kind='素选'):
        # 人数未满才返回classid
        classtype = "XGXK"
        if kind == '素选':
        ...
        while True:
            try:
                ...
                for course in datalist:
                    ...
                    if remain > 0 and course['teacherName'] == teacher and course['teachingClassID'] == classID:
                        string = f'{course_name} {teacher} {course["teachingClassID"]}：{remain}人空缺'
                        print(string)
                        print(f'{course["teachingClassID"]}:and classid')
                        to_wechat(key, f'{course_name} 余课提醒', string)
                        res = self.post_add(course_name, teacher, classtype, course['teachingClassID'], key)
                        return res

                print(f'{course_name} {teacher} {classID}：人数已满 {time.ctime()}')
                time.sleep(1)
                ...
                to_wechat(key, '登录失效，请重新登录', '线程结束')
                return False
    def judgepr(self, course_name, teacher, key='', kind='素选'):
        # 人数未满才返回classid
        classtype = "XGXK"
        if kind == '素选':
            kind = 'publicCourse.do'
